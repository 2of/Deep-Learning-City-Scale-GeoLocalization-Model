...
# Load the CSV file with latitude and longitude points
df = pd.read_csv('./data/GeoJSON/image_data_chicago.csv')

# Extract latitude and longitude columns
points_lat = df['latitude'].tolist()
points_lon = df['longitude'].tolist()
...
fig.add_trace(go.Scattermapbox(
    ...
    lon=longitudes,
    lat=latitudes,
    line=dict(color='blue', width=2)  # Bounding box outline
))

# Add the points from the CSV file
fig.add_trace(go.Scattermapbox(
    mode='markers',
    lon=points_lon,
    lat=points_lat,
    marker=dict(color='red', size=5),  # Red markers for the points
    name="Points"
))
