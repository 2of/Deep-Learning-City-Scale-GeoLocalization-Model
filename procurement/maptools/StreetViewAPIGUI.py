...
class StreetViewDownloaderApp(QtWidgets.QWidget):
    ...
    def download_all(self):
        ...
        download_dir = os.path.join("data", timestamp)
        os.makedirs(download_dir, exist_ok=True)

        # CSV to log the downloads
        ...
        for i in range(self.total_to_dl):
            ...
            url = self.generate_streetview_url(lat, lon, key)
            ...
            if response.status_code == 200:
                ...
                # Log the download
                log_data.append({"File Name": file_name, "Latitude": lat, "Longitude": lon})
        ...
        if self.radio2.isChecked():  # Download Single
            ...
            if current_row_index != -1:  # Check if a row is selected
                ...
                if response.status_code == 200:
                    ...
                    # Log the download
                    log_data = [{"File Name": file_name, "Latitude": lat, "Longitude": lon}]
                    log_df = pd.DataFrame(log_data)
                    log_file = os.path.join(download_dir, "download_log.csv")
                    log_df.to_csv(log_file, index=False)

                    QMessageBox.information(self, "Download Complete", f"Image downloaded and saved to {file_path}")
                else:
    ...
    def check_url(self):
        ...
        if current_row_index != -1:  # Check if a row is selected
            ...
            lon = row_data[1]
            key = self.api_key_entry.text()
            res = self.generate_streetview_url(lat, lon, key)
            QMessageBox.information(self, "URL", res)
        else:
            QMessageBox.warning(self, "Warning", "No row selected to check URL.")

    def generate_streetview_url(self, latitude, longitude, key, size="640x640", heading=0, pitch=0):
        """
        ...
        - size (str): The size of the image (default is '640x640').
